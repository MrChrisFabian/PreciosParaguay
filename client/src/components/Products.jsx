...
const ProductsP = ({ title, price, url, image_url, id }) => {
  const [imageWeb, setImageWeb] = useState({ url: null, alt: null })
  ...
  const getImageWeb = async () => {
    if (url && url.includes('visuar.com.py')) {
      setImageWeb({ url: '/public/images/visuar.png', alt: 'Visuar Logo' })
    }
    else if (url && url.includes('pulsogamer.com.py')) {
      setImageWeb({ url: '/public/images/pulso.png', alt: 'PulsoGamer Logo' })
    } else {
      setImageWeb({ url: '/public/images/nissei.webp', alt: 'Nissei Logo' })
    }
  }
    ...
    <div class=" flex flex-col w-full h-full max-w-sm  border border-gray-200 rounded-lg shadow-lg  dark:border-gray-700">
      ...
      <div className=" p-4  bg-white">
        ...
        <div className="flex flex-col items-center content-center">
          {/* img de la marca */}
          <a href={url} target="_blank"><img className="my-8" src={imageWeb.url} alt={imageWeb.alt} /></a>
          {/* Botones y precio */}
          <div className="flex items-center content-center justify-between">
            <div class="flex items-center">
              <span class="text-3xl font-bold text-gray-900 dark:text-white mr-10">{price}₲</span>
